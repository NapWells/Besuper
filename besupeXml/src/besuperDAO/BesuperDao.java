...
public class BesuperDao extends DBConnection{
    ...
    public Player signup(PlayerCredential player) {
        ...
        try {
            ...
            statement.setString(3,DigestUtils.md5Hex(player.getPassword()));
    ...
    public boolean isUsernameExists(String username){
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement("select * from player where username = ?");
    ...
    public Player login(PlayerCredential credential) {
        ...
        try {
            ...
            statement.setString(2, DigestUtils.md5Hex(credential.getPassword()));
    ...
    public void updatePlayer(Player player) {
        String sql = "UPDATE `player` SET `name`=?, `score`=? WHERE `id`=?";
    ...
    public void upsertQuestion(Question question) {
        ...
        String sqlInsert = "INSERT INTO `question` (`value`, `credit`, `player_id`, `isOpen`) VALUES (?,?,?,?)";
        String sqlUpdate = "UPDATE `question` SET `value`=?, `credit`=?, `player_id`=?, `isOpen`=? WHERE `id`=?";
