...
<div id="wrapper">
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
            <ul class="nav navbar-nav side-nav">
                ...
                <li>
                    <a href="<%=basePath%>riderList.action" class="active"><i class="fa fa-fw fa-table"></i>骑士管理</a>
                </li>
                <li>
    ...
    <div id="page-wrapper">
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-lg-6">
                    <form method="post" action="riderList.action" class="form-group input-group" id="search-from-list">
                        <input type="text" class="form-control"  name="search" placeholder="搜索相关骑士信息">
                        <span class="input-group-btn"><button class="btn btn-default" onclick="submitForm()" type="button"><i
                ...
                <div class="col-lg-6">
                    ...
                    <table class="table table-bordered table-striped" id="rider-info-table">
                        ...
                        <tbody>
                        <c:forEach items="${riderMap}" var="rider" varStatus="status">
                            <tr>
                                <form action="changeRiderStatus.action" id="updateForm${status.index}" method="post" style="display: none">
                                    <input type="hidden" name="id" value="${rider.key.id}">
                                    <input type="hidden" name="riderStatusId" value="${rider.key.riderStatusId}">
                                </form>
                                <td class="sName">${rider.key.name}</td>
                                <td class="phone">${rider.key.phone}</td>
                                <td class="time"><fmt:formatDate value="${rider.key.applyTime}"
                                                                 pattern="yyyy-MM-dd"/></td>
                                <td class="status">${rider.value.name}</td>
                                <td class="action">
                                    <c:if test="${rider.value.name=='已通过'}">
                                        <button type="submit" onclick="updateRiderStatus('${status.index}')" class="btn btn-default">中止</button>
                                    </c:if>
                                    <c:if test="${rider.value.name=='未通过'}">
                                        <button type="submit" onclick="updateRiderStatus('${status.index}')" class="btn btn-default warning">通过</button>
                                    </c:if>
                                </td>
                            </tr>
                        </c:forEach>
...
<script src="<%=basePath%>resources/js/riders.js"></script>
